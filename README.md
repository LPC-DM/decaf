...
You can also plot the inputs from a **single** workspace with the `makeWorkspacePlots.C` macro.
It stacks all the inputs in a single histogram.
Copy it to wherever the ROOT files are (usually they will be in the `darkhiggs` folder you created just above) and do, for example:
```
...
the second argument (`5000`) is the limit of the y-axis of the stack;
the third argument (`2000`) is the y-position of an explanatory label; that label will simply be the same as the first argument again.
