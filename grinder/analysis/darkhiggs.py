#!/usr/bin/env python
#This can be moved into the executor description maybe
...
class darkhiggsProcessor(processor.ProcessorABC):
    def __init__(self, debug=False):
        ...
        jet_selection_axis = hist.Cat("jet_selection","JetSelection")

        hists['sumw'] = hist.Hist("sumw", dataset_axis, hist.Bin("sumw", "Weight value", [0.]))
        hists['CaloMinusPfOverRecoil'] = hist.Hist("Events", dataset_axis, region_axis, jet_selection_axis, hist.Bin("CaloMinusPfOverRecoil","Calo - Pf / Recoil",35,0,1))
        hists['recoil'] = hist.Hist("Events", dataset_axis, region_axis, jet_selection_axis, hist.Bin("recoil","Hadronic Recoil",[250.0, 280.0, 310.0, 340.0, 370.0, 400.0, 430.0, 470.0, 510.0, 550.0, 590.0, 640.0, 690.0, 740.0, 790.0, 840.0, 900.0, 960.0, 1020.0, 1090.0, 1160.0, 1250.0]))
        hists['mindphi'] = hist.Hist("Events", dataset_axis, region_axis, jet_selection_axis, hist.Bin("mindphi","Min dPhi(MET,AK4s)",15,0,6.28))
        hists['j1pt'] = hist.Hist("Events", dataset_axis, region_axis, jet_selection_axis, hist.Bin("j1pt","AK4 Leading Jet Pt",[30.0, 60.0, 90.0, 120.0, 150.0, 180.0, 210.0, 250.0, 280.0, 310.0, 340.0, 370.0, 400.0, 430.0, 470.0, 510.0, 550.0, 590.0, 640.0, 690.0, 740.0, 790.0, 840.0, 900.0, 960.0, 1020.0, 1090.0, 1160.0, 1250.0]))
        hists['j1eta'] = hist.Hist("Events", dataset_axis, region_axis, jet_selection_axis, hist.Bin("j1eta","AK4 Leading Jet Eta",35,-3.5,3.5))
        hists['j1phi'] = hist.Hist("Events", dataset_axis, region_axis, jet_selection_axis, hist.Bin("j1phi","AK4 Leading Jet Phi",35,-3.5,3.5))
        hists['fj1pt'] = hist.Hist("Events", dataset_axis, region_axis, jet_selection_axis, hist.Bin("fj1pt","AK15 Leading Jet Pt",[200.0, 250.0, 280.0, 310.0, 340.0, 370.0, 400.0, 430.0, 470.0, 510.0, 550.0, 590.0, 640.0, 690.0, 740.0, 790.0, 840.0, 900.0, 960.0, 1020.0, 1090.0, 1160.0, 1250.0]))
        hists['fj1eta'] = hist.Hist("Events", dataset_axis, region_axis, jet_selection_axis, hist.Bin("fj1eta","AK15 Leading Jet Eta",35,-3.5,3.5))
        hists['fj1phi'] = hist.Hist("Events", dataset_axis, region_axis, jet_selection_axis, hist.Bin("fj1phi","AK15 Leading Jet Phi",35,-3.5,3.5))
        hists['njets'] = hist.Hist("Events", dataset_axis, region_axis, jet_selection_axis, hist.Bin("njets","AK4 Number of Jets",6,0,5))
        hists['nfjets'] = hist.Hist("Events", dataset_axis, region_axis, jet_selection_axis, hist.Bin("nfjets","AK15 Number of Jets",4,0,3))
        hists['fjmass'] = hist.Hist("Events", dataset_axis, region_axis, jet_selection_axis, hist.Bin("fjmass","AK15 Jet Mass",50,20,250))
        hists['e1pt'] = hist.Hist("Events", dataset_axis, region_axis, jet_selection_axis, hist.Bin("e1pt","Leading Electron Pt",[30.0, 60.0, 90.0, 120.0, 150.0, 180.0, 210.0, 250.0, 280.0, 310.0, 340.0, 370.0, 400.0, 430.0, 470.0, 510.0, 550.0, 590.0, 640.0, 690.0, 740.0, 790.0, 840.0, 900.0, 960.0, 1020.0, 1090.0, 1160.0, 1250.0]))
        hists['e1eta'] = hist.Hist("Events", dataset_axis, region_axis, jet_selection_axis, hist.Bin("e1eta","Leading Electron Eta",48,-2.4,2.4))
        hists['e1phi'] = hist.Hist("Events", dataset_axis, region_axis, jet_selection_axis, hist.Bin("e1phi","Leading Electron Phi",64,-3.2,3.2))
        hists['mu1pt'] = hist.Hist("Events", dataset_axis, region_axis, jet_selection_axis, hist.Bin("mu1pt","Leading Muon Pt",[30.0, 60.0, 90.0, 120.0, 150.0, 180.0, 210.0, 250.0, 280.0, 310.0, 340.0, 370.0, 400.0, 430.0, 470.0, 510.0, 550.0, 590.0, 640.0, 690.0, 740.0, 790.0, 840.0, 900.0, 960.0, 1020.0, 1090.0, 1160.0, 1250.0]))
        hists['mu1eta'] = hist.Hist("Events", dataset_axis, region_axis, jet_selection_axis, hist.Bin("mu1eta","Leading Muon Eta",48,-2.4,2.4))
        hists['mu1phi'] = hist.Hist("Events", dataset_axis, region_axis, jet_selection_axis, hist.Bin("mu1phi","Leading Muon Phi",64,-3.2,3.2))
        hists['TvsQCD'] = hist.Hist("Events", dataset_axis, region_axis, jet_selection_axis, hist.Bin("TvsQCD","TvsQCD",15,0,1))
        hists['hSvsQCD'] = hist.Hist("Events", dataset_axis, region_axis, jet_selection_axis, hist.Bin("hSvsQCD","hSvsQCD",15,0,1))
        hists['VvsQCD'] = hist.Hist("Events", dataset_axis, region_axis, jet_selection_axis, hist.Bin("VvsQCD","VvsQCD",15,0,1))
    ...
    def process(self, tree):
        # do the things                                                                                                                                      
        dataset = df['dataset']
        #also need selected_regions, year, xsec, file
        weight = {}
        #tree = uproot.open(file)["Events"] <- Need to change tree.array to tree['']
        genw = 1
        sumw = 1
    ...
    ###
        ...
        for path in singleele_trigger_paths[year]:
            if path in tree:
                singleele_trigger[path] = tree[path]
        ...
        for path in singlepho_trigger_paths[year]:
            if path in tree:
                singlepho_trigger[path] = tree[path]
    ...
    ###
        ...
        fj = Initialize({'pt':tree['AK15Puppi_pt'],
                         'eta':tree['AK15Puppi_eta'],
                         'phi':tree['AK15Puppi_phi'],
                         'mass':tree['AK15Puppi_mass'],
                         'id':tree['AK15Puppi_jetId'],
                         'probTbcq':tree['AK15Puppi_probTbcq'],
                         'probTbqq':tree['AK15Puppi_probTbqq'],
                         'probTbc':tree['AK15Puppi_probTbc'],
                         'probTbq':tree['AK15Puppi_probTbq'],
                         'probWcq':tree['AK15Puppi_probWcq'],
                         'probWqq':tree['AK15Puppi_probWqq'],
                         'probZbb':tree['AK15Puppi_probZbb'],
                         'probZcc':tree['AK15Puppi_probZcc'],
                         'probZqq':tree['AK15Puppi_probZqq'],
                         'probHbb':tree['AK15Puppi_probHbb'],
                         'probHcc':tree['AK15Puppi_probHcc'],
                         'probHqqqq':tree['AK15Puppi_probHqqqq'],
                         'probQCDbb':tree['AK15Puppi_probQCDbb'],
                         'probQCDcc':tree['AK15Puppi_probQCDcc'],
                         'probQCDb':tree['AK15Puppi_probQCDb'],
                         'probQCDc':tree['AK15Puppi_probQCDc'],
                         'probQCDothers':tree['AK15Puppi_probQCDothers']})
        ...
        met = Initialize({'pt':tree['MET_pt'],
                          'eta':0,
                          'phi':tree['MET_phi'],
                          'mass':0})
        
        calomet = Initialize({'pt':tree['CaloMET_pt'],
                              'eta':0,
                              'phi':tree['CaloMET_phi'],
                              'mass':0})
        
        weight["nlo"] = 1
        if xsec != -1:
            gen = Initialize({'pt':tree['GenPart_pt'],
                              'eta':tree['GenPart_eta'],
                              'phi':tree['GenPart_phi'],
                              'mass':tree['GenPart_mass'],
                              'pdgid':tree['GenPart_pdgId'],
                              'status':tree['GenPart_status'], 
                              'flags':tree['GenPart_statusFlags'],
                              'motherid':tree['GenPart_genPartIdxMother']})
...
if __name__ == '__main__':
    processor_instance = darkhiggsProcessor()

    with lz4f.open('DarkHiggsProcessor.cpkl.lz4', mode='wb', compression_level=5 ) as fout:
