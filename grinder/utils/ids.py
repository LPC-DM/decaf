...
e_id['2018'] = e_id['2016']
e_id['2018']['loose_id'] = 'Electron_cutBased'
e_id['2018']['tight_id'] = 'Electron_cutBased'
e_id['2018']['iso'] = 'Null'
...
def isLooseElectron(pt,eta,dxy,dz,iso,loose_id,year):
    ...
    if year=='2016':
        ...
        #mask = ((pt>10)&(abs(eta)<2.5)&((dxy<0.1)*(dxy>0.05))&((dz<0.2)*(dz>0.1))&(loose_id>=1)) 
        #dxy 0.05 / 0.10     dz 0.1 / 0.2
    return mask
...
mu_id['2018']['tight_id'] = 'Muon_looseId'

def isLooseMuon(pt,eta,dxy,dz,iso,loose_id,year):
...
tau_id['2018']['id'] = 'Tau_idMVAnewDM2017v2'
...
pho_id['2018']['loose_id'] = 'Photon_cutBasedBitmap'
pho_id['2018']['tight_id'] = 'Photon_cutBasedBitmap'
pho_id['2018']['eleveto']  = 'Photon_electronVeto'
pho_id['2018']['phoeta']  = 'Photon_eta'
...
def isLoosePhoton(pt,eta,loose_id,eleveto,phoeta,year):
    mask = ~(pt==np.nan)#just a complicated way to initialize a jagged array with the needed shape to True
    if year=='2016':
        ...
        mask = (pt>15)&(abs(eta)<2.5)&((loose_id&2)>=1)&(eleveto)&(abs(phoeta)<1.479)
    return mask

def isTightPhoton(pt,eta,tight_id,eleveto,phoeta,year):
    mask = ~(pt==np.nan)#just a complicated way to initialize a jagged array with the needed shape to True
    if year=='2016':
        ...
        mask = (pt>215)&(abs(eta)<1.4442)&((tight_id&2)==2)&(eleveto)&(abs(phoeta)<1.479) # Trigger threshold is at 200
    return mask
