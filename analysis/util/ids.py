...
def isTightElectron(pt,eta,tight_id,year):
    mask = ~(pt==np.nan)#just a complicated way to initialize a jagged array with the needed shape to True
    if year=='2016':
        mask = ((pt>29)&(abs(eta)<2.5)&(tight_id==4)) # Trigger: HLT_Ele27_WPTight_Gsf_v
    elif year=='2017':
        mask = ((pt>34)&(abs(eta)<2.5)&(tight_id==4)) # Trigger: HLT_Ele35_WPTight_Gsf_v
    elif year=='2018':
        mask = ((pt>34)&(abs(eta)<2.5)&(tight_id==4)) # Trigger: HLT_Ele32_WPTight_Gsf_v
    return mask
