    ...
    for year in ['2016','2017','2018']:
        for mass in ['mass0','mass1','mass2','mass3','mass4']:
            for category in ['monojet','monohs']:
                ...
                for filename in os.listdir(options.analysis+year+'/'+mass):
                    if '.txt' not in filename: continue
                    if category not in filename: continue
                    command=command+filename.split(".")[0]+'='+options.analysis+year+'/'+mass+'/'+filename+' '
    command=command+' > '+options.analysis+options.year+options.mass+options.category+'.txt'
    os.system(command)
