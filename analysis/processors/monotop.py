...
class AnalysisProcessor(processor.ProcessorABC):
    ...
    def __init__(self, year, xsec, corrections, ids, common):
        ...
        self._samples = {
            'sr':('ZJets','WJets','DY','TT','ST','WW','WZ','ZZ','QCD','HToBB','MET','Vector_MonoTop'),
    ...
    def process(self, events):
        ...
        fj['TvsQCD'] = (fj.probTbcq + fj.probTbqq) / (fj.probTbcq + fj.probTbqq + fj.probQCDbb+fj.probQCDcc+fj.probQCDb+fj.probQCDc+fj.probQCDothers)
        ...
        def fill(dataset, region, systematic, weight, cut):
            ...
            for histname, h in hout.items():
                if not isinstance(h, hist.Hist):
                    ...
                    h.fill(dataset=dataset, region=region, systematic=sname, recoil=u[region.split('_')[0]].mag, mindphi=abs(u[region.split('_')[0]].delta_phi(j_clean.T)).min(), weight=weight*cut)
                else:
...
if __name__ == '__main__':
    ...
    save(processor_instance, 'data/monotop'+options.year+'.processor')
