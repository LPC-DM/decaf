...
def plotPreFitPostFit(region, year, signalflag, recoil, blind=False):
    ...
    ### Call data (TGraphAsymmErrors) ###
    h_data = f_mlfit.Get("shapes_fit_b/"+darkhiggs_regions[region]+years[year]+signalprocess[signalflag]+recoilbin[recoil]+"/data")
    ...
    # Pre-Fit
    h_prefit = {}
    #print("shapes_prefit/"+darkhiggs_regions[region]+years[year]+signalprocess[signalflag]+recoilbin[recoil]+"/total")
    h_prefit['total'] = f_mlfit.Get("shapes_prefit/"+darkhiggs_regions[region]+years[year]+signalprocess[signalflag]+recoilbin[recoil]+"/total")
    ...
    for process in processes:
        #print("shapes_prefit/"+darkhiggs_regions[region]+years[year]+signalprocess[signalflag]+recoilbin[recoil]+"/"+process)
        print('which process ?', process)
        h_prefit[process] = f_mlfit.Get("shapes_prefit/"+darkhiggs_regions[region]+years[year]+signalprocess[signalflag]+recoilbin[recoil]+"/"+process)
    ...
    # Post-Fit
    h_postfit = {}
    h_postfit['total'] = f_mlfit.Get("shapes_fit_b/"+darkhiggs_regions[region]+years[year]+signalprocess[signalflag]+recoilbin[recoil]+"/total")
    ...
    h_postfit['totalv2'] = f_mlfit.Get("shapes_fit_b/"+darkhiggs_regions[region]+years[year]+signalprocess[signalflag]+recoilbin[recoil]+"/total_background")
    ...
    for process in processes:
        h_postfit[process] = f_mlfit.Get("shapes_fit_b/"+darkhiggs_regions[region]+years[year]+signalprocess[signalflag]+recoilbin[recoil]+"/"+process)
    ...
    for i in range(1,h_all_prefit.GetNbinsX()+1):
        ...
        if not blind:
            ndata = h_data.GetPointY(i)
        else:
            ndata = 0
        #print ndata

        if (ndata>0.0 and not blind):
            e_data_hi = h_data.GetErrorYhigh(i)
            e_data_lo = h_data.GetErrorYlow(i)
        else:
            e_data_hi = 0.0
            e_data_lo = 0.0
...
#### Call Functions ####
dh_regions = ['singlemw', 'singlemtop', 'singleew', 'singleetop', 'photon', 'diele', 'dimu']
sigs = ['mhs', 'mjet']
