...
def plotPreFitPostFit(region, year, signalflag, recoil, blind=False):
    ...
    ### Check directory ###
    h_dir = f_mlfit.Get(darkhiggs_regions[region]+years[year]+signalprocess[signalflag]+recoilbin[recoil]+"_postfit")

    if h_dir == None:
        raise RuntimeError

    ### Call data ###
    h_data = f_mlfit.Get(darkhiggs_regions[region]+years[year]+signalprocess[signalflag]+recoilbin[recoil]+"_postfit/data_obs")
