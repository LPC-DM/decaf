...
sub = re.sub
def cleanName(name):
  return sub('CR','',sub('_bin','',sub('monotop_','',sub('stat_error','stat',sub('Model','_',name)))))
            ...
            if nuis_p != None:
                if options.plotfile:
                    if fit_name=='b':
                        ...
                        hist_fit_b.GetXaxis().SetBinLabel(nuis_p_i,cleanName(name))
                    if fit_name=='s':
                        hist_fit_s.SetBinContent(nuis_p_i,nuis_x.getVal())
                        hist_fit_s.SetBinError(nuis_p_i,nuis_x.getError())
                        hist_fit_s.GetXaxis().SetBinLabel(nuis_p_i,cleanName(name))
                        hist_prefit.SetBinContent(nuis_p_i,mean_p)
                        hist_prefit.SetBinError(nuis_p_i,sigma_p)
                        #hist_prefit.SetBinError(nuis_p_i,1)
                        hist_prefit.GetXaxis().SetBinLabel(nuis_p_i,cleanName(name))
...
if options.plotfile:
    ...
    histogram = ROOT.TH1F("pulls", "Pulls", 60, -2, 2)
    for pull in pulls:
        histogram.Fill(pull)
    canvas = ROOT.TCanvas("asdf", "asdf", 800, 800)
    histogram.GetXaxis().SetTitle("pull")
    histogram.GetYaxis().SetTitle("Number of nuisances")
    histogram.SetTitle("")
    ...
    for ext in ['pdf','png','C']:
      canvas.SaveAs(plotConfig.plotDir+'plotsDist.'+ext)
    ...
    canvas_nuis.SaveAs(plotsDir+"pulls.C")
