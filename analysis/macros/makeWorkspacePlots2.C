...
TH1 *getDataPlot(std::string fileName, std::string wsname, std::string catName) {
  ...
  /// Now we do a bit of magic
  std::string fullName = std::string("shapeBkg_") + "_data_" + wsname + "Pdf" + "__fjmass";
  std::string histName = "hist_" + fullName;
  std::string histTitle = "Histogram of " + histName;
  dataPlot->SetName(histName.c_str());
  dataPlot->SetTitle(histTitle.c_str());
  dataPlot->SetEntries(dataPlot->Integral());
...
void makeWorkspacePlots2(const std::string name, int histoTop, int latexY) {
  ...
  std::string outRootName = std::string("shapeBkg_") + "data_"
    + name + std::string("Pdf__fjmass.root");
  ...
  TH1F *histoClone = (TH1F*)dataPlot->Clone();
  TFile* tFile = TFile::Open(outRootName.c_str(),"RECREATE");
  histoClone->Write();
  tFile->Close();
