...
if __name__ == "__main__":
    ...
    for recoilbin in range(nrecoil):
        ...
        MCbkgList_pass = ["Z+jets", "W+jets", "ST", "DY+jets", "VV", "Hbb", "QCD"]
        if isttMCpass: MCbkgList_pass.append("TT")
        sr_central_pass, sr_error2_pass = get_mergedMC_stat_variations(background, recoilbin, "sr", "pass", mass, MCbkgList_pass)
        sr_central_fail, sr_error2_fail = get_mergedMC_stat_variations(background, recoilbin, "sr", "fail", mass, ["W+jets", "TT", "ST", "DY+jets", "VV", "Hbb", "QCD"])
        ...
        sr_zjetsFailBinYields = np.array([rl.IndependentParameter(
            "sr" + year + "fail" + "mass" + mass + "recoil" + str(recoilbin) + "_zjets_mu"+str(b), 
            sr_zjetsMCFailTemplate[0][b], 
            1e-5, 
            sr_zjetsMCFailTemplate[0].max()*2) for b in range(len(sr_zjetsMCFailTemplate[0]))])
