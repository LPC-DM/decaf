...
parser.add_option('-k', '--packed', action="store_true", dest="packed")
...
for folder in beans[options.year]:
    ...
    for dataset in xsections.keys():
        ...
        if options.packed: filenames = folder+"/"+dataset+" -name \'"+dataset+"*.root\'"
        ...
        urllists = urllist
        if not options.packed: urllists = split(urllist, int(options.pack))
        print(len(urllists))
        if urllist:
...
if options.packed: folder = "beans/packed"+options.year+".json"
